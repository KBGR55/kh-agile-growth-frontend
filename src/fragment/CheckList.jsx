    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.code === 201) {
                ...
                try {
                    const resCategoria = await peticionGet(getToken(), `/resultado_categoria/calcular/${external_id}`);
                    console.log("Respuesta de resultado_categoria:", resCategoria);
    
                    if (resCategoria.code === 200) {
                        const resMadurez = await peticionGet(getToken(), `/nivel_madurez_general/calcular/${external_id}`);
                    ...
                    await swal("Error crítico", "No se pudo calcular los resultados.", "error");
                }
            } else {
                await swal("Error", response.msg || "Hubo un problema al guardar las respuestas.", "error");
            }
        } catch (error) {
            console.error("Error crítico al guardar las respuestas:", error);
            await swal("Error crítico", "No se pudo establecer conexión con el servidor.", "error");
        }
    };
